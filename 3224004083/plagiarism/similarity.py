...
4. **多阶融合**：短片段（n=1,2）对局部改写、逐字插入、字符乱序更鲁棒，长片段
   （n=3,4）更能区分"内容相同只是乱序"和"两篇无关中文"，因此按
   0.30/0.30/0.25/0.15 加权（在官方样例上实测调出来的，见下方 NGRAM_WEIGHTS）。
...
# 各阶 n-gram 的权重。
#
# 为什么低阶占主导：这道题的抄袭手段是"在原文字基础上增、删、改、乱序"——
# 逐字插入会打断局部顺序（高阶 n-gram 崩），字符级乱序更会把高阶片段彻底打散。
# 真正稳定不变的是"字符的频次分布"（1-gram 与 2-gram），所以把权重向低阶倾斜，
# 才能对增删改乱序都给出稳定高分；同时保留 3、4 阶的一定权重，用来区分
# "内容相同只是乱序"和"两篇无关中文"（无关文本的高阶 n-gram 几乎为零）。
# 该权重是在官方样例（orig_0.8_add/del/dis_1~15）上实测调出来的：
# 增/删 ≈ 0.80、段落乱序 ≈ 0.95、重度乱序 ≈ 0.64、无关文本 ≈ 0.09。
NGRAM_WEIGHTS: dict[int, float] = {1: 0.30, 2: 0.30, 3: 0.25, 4: 0.15}
