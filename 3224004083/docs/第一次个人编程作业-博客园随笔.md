...
| 这个作业的目标 | 实现论文查重程序，练习 PSP 时间管理、GitHub 代码管理、性能分析、单元测试与代码质量分析 |
...
> 说明：**「预估耗时」在动手写代码之前填写**，「实际耗时」在项目全部完成后回填。
> 预估不是拍脑袋填的，而是先列任务清单再逐项估时；实际耗时的偏差分析见第九章。
...
**为什么拆成这几个模块**：文件读写与相似度计算**完全分离**（`reader.py`/`writer.py`
负责磁盘，`similarity.py` 只吃字符串），这样相似度函数可以脱离磁盘做纯函数测试，
也让"不能读写其他文件"这条约束在代码结构上就看得出来——全项目只有两处 `open()`。
...
| orig_0.8_syn | 近义替换（改） | 0.9353 | 0.8789 | **0.9396** |
| orig_0.8_dis_1 | 乱序：段内句子旋转 | 0.9746 | 0.9508 | **0.9767** |
| orig_0.8_dis_2 | 乱序：整体打乱 | 0.9347 | 0.8792 | **0.9401** |
| orig_0.8_dis_3 | 乱序：句内分句颠倒 | 0.9090 | 0.8366 | **0.9164** |
| orig_0.8_add | 插入新句子（增） | 0.8862 | 0.7957 | **0.9120** |
| orig_0.8_del | 删除句子（删） | 0.8671 | 0.7655 | **0.8955** |
| orig_0.0 | 完全不同主题 | 0.0157 | 0.0084 | **0.0364** |

三点结论：

1. 三者方向完全一致，**差异都在 0.03 以内**，不足以决定成败；
2. 余弦对**文本长度不敏感**——把整篇原文复制几遍，得分仍然是 1。好处是删减版、
   分段版仍然能被识别；代价是"单纯插入大段无关内容"的惩罚比另外两种度量略小；
3. Dice / Jaccard 对"包含关系"更敏感，无关文本压得更低。
...
- **HTML 噪声剥离**：评测样例里有部分内容是从网页抓下来的，夹带了 `<html>/<div>/
  ...
  0.60 左右；剥离后回到 **0.99**（见下表 `orig_1.0_html` 一行）。

| 样例 | 说明 | 相似度 |
|---|---|---|
| orig_1.0 | 与原文完全一致 | 1.0000 |
| orig_1.0_html | 正文相同，外面包了一层网页外壳 | **0.9941** |
| orig_0.8_syn | 术语替换为近义词（改） | 0.9396 |
| orig_0.8_dis_1 | 段内句子局部旋转（乱序） | 0.9767 |
| orig_0.8_dis_2 | 全部句子整体打乱（乱序） | 0.9401 |
| orig_0.8_dis_3 | 句子内部分句顺序颠倒（乱序） | 0.9164 |
| orig_0.8_add | 插入约 20% 新句子（增） | 0.9120 |
| orig_0.8_del | 删除约 20% 句子（删） | 0.8955 |
| orig_0.8_del_gbk | 同上，但文件是 GB18030 编码 | 0.8955 |
| orig_0.0 | 完全不同主题 | 0.0364 |
...
短片段对局部改写（同义替换、换词）敏感，长片段更能体现稳定内容、对无关文本更有
区分度，两者互补。
...
| 实现方式 | 耗时 | 相对提速 |
|---|---|---|
| 优化前：逐字符 Python 循环 | 0.389 s | 1.00× |
| 优化后：查表 + `str.translate` | 0.101 s | **3.85×** |
...
| 函数 | tottime 占比 |
|---|---|
| `Counter._count_elements`（统计频次） | 48.4% |
| `<genexpr>`（切 n-gram 片段） | 33.6% |
| `unicodedata.normalize`（NFKC） | 10.3% |
| `_translation_for`（建翻译表） | 3.5% |
| `str.translate`（规范化） | 3.1% |
...
|---|---|---|---|---|---|---|
| 样例级 | 5 541 | 4 724 | 18.1 ms | 19.1 ms | 51.7 ms | 0.2 MiB |
| 中篇 | 55 386 | 47 074 | 158.5 ms | 162.6 ms | 180.4 ms | 0.7 MiB |
| 长篇 | 184 616 | 156 831 | 534.2 ms | 597.9 ms | 618.4 ms | 2.4 MiB |
| 极端 | 553 847 | 470 860 | 1711.5 ms | 1739.9 ms | 1812.2 ms | 7.1 MiB |

即便 55 万字符的极端输入（约 1.1 MB 文本，远大于课程样例），也在 **1.8 秒内、
8 MiB 以内**完成，距离 5 秒 / 2048 MB 的限制有很大余量；运行时间与文本长度基本
...
**73 个用例全部通过，216 条语句 0 条未覆盖，语句覆盖率 100%**。
...
**还不够的部分**：相似度是**字面**相似度，不理解语义，所以"星期天 ↔ 周天"这类
同义改写只能靠字符重合来识别（实测 0.94，靠的是"天""晴"这些字）；没有标注数据，
所以 n-gram 的权重（0.10/0.20/0.35/0.35）是手工设定、缺少验证；也没有做"分数分布"
层面的评测（比如 100 对无关论文的最高分是多少）。这三点写在第九章的改进计划里。
...
| 极端输入（55 万字符） | 1.74 s / 7.1 MiB，远低于 5 s / 2048 MB |
| 网络访问 | 无（只 import 标准库） |
...
3. **权重缺少数据支撑**。1~4 阶的权重（0.10/0.20/0.35/0.35）是手工设定的。
