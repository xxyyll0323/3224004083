...
> 说明：**「预估耗时」在动手写代码之前填写**，「实际耗时」在项目全部完成后回填（见第九章）。
> 这里的预估不是拍脑袋填的，而是先列任务清单再逐项估时。
...
![性能分析图：热点函数](【上传 docs/perf_chart.png 后替换此链接】)
...
![规范化改进前后对比](【上传 docs/normalize_compare.png 后替换此链接】)
...
![单元测试覆盖率](【上传 docs/coverage_chart.png 后替换此链接】）
